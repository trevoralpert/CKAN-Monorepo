...
class SearchEnhancedController(ApiController):
    ...
    def related_datasets(self):
        ...
        try:
            ...
            try:
                pkg_dict = toolkit.get_action('package_show')(context, {'id': dataset_id})
    ...
    def _get_analytics_suggestions(self, query: str, limit: int = 5) -> List[str]:
        ...
        try:
            ...
            suggestions = model.Session.query(
                ...
                model.desc('frequency')
            ).limit(limit).all()
            
            return [sugg[0] for sugg in suggestions if sugg[0] and len(sugg[0]) > len(query)]
            
        except Exception as e:
            log.debug(f"Analytics suggestions failed: {e}")
            return []

    def _get_autocomplete_suggestions(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    def _calculate_related_datasets(self, pkg_dict: Dict[str, Any], limit: int = 5) -> List[Dict[str, Any]]:
