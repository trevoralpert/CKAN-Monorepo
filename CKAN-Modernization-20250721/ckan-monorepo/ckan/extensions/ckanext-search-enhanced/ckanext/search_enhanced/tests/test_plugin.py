...
from ckanext.search_enhanced.solr_enhancements import SolrSchemaEnhancer, enhance_dataset_for_indexing
...
class TestSolrSchemaEnhancer:
    ...
    def test_get_enhanced_fields_includes_city_fields(self):
        ...
        expected_fields = [
            'department', 'update_frequency', 'geographic_coverage',
            'data_quality_assessment', 'public_access_level',
            'contact_email', 'last_updated', 'collection_method',
            'popularity_score', 'view_count_30d'
        ]
...
class TestEnhanceDatasetForIndexing:
    ...
    def test_enhance_dataset_with_analytics(self, mock_analytics_event, mock_model):
        ...
        # Mock analytics data
        mock_model.Session.query.return_value.filter.return_value.count.return_value = 25
...
class TestSearchEnhancedController:
    ...
    def test_search_suggestions_endpoint(self):
        ...
        with patch('ckanext.search_enhanced.controller.toolkit') as mock_toolkit:
            ...
            with patch.object(controller, '_get_analytics_suggestions') as mock_analytics:
                with patch.object(controller, '_get_autocomplete_suggestions') as mock_autocomplete:
                    mock_analytics.return_value = ['fire department', 'fire safety']
                    mock_autocomplete.return_value = [{'title': 'Fire Stats', 'name': 'fire-stats'}]
